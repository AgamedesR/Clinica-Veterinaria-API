...
import { consultaCreateSchema, consultaUpdateSchema } from "../validators/consulta.validator";
...
export const consultaController = {
  ...
  create: async (req: Request, res: Response) => {
    try {
      ...
      const consulta = await consultaService.create({
        ...parsed,
        dataHora: new Date(parsed.dataHora),
      });
  ...
  update: async (req: Request, res: Response) => {
    try {
      ...
      const updated = await consultaService.update(id, {
        ...parsed,
        dataHora: parsed.dataHora ? new Date(parsed.dataHora) : undefined,
      });
      res.json(updated);
    } catch (error: any) {
      if (error instanceof ZodError) {

...
import { ZodError } from 'zod'; // Necessário para tratamento de erro Zod
...
export const pacienteController = {
   ...
   * 409:
   * description: CPF ou Email já cadastrado (Prisma P2002)
   */
  create: async (req: Request, res: Response) => {
    try {
      // Zod faz a validação E a transformação da data (como combinado)
      ...
      if (error instanceof ZodError) {
        // Tratamento Zod (igual ao consultaController)
        res.status(400).json({ errors: error.flatten().fieldErrors });
      } else if (error.code === 'P2002') {
        // Tratamento Prisma (Duplicidade)
   ...
   * description: Dados inválidos (Zod Error)
   * 404:
   * description: Paciente não encontrado (Prisma P2025)
   */
   ...
   * responses:
   * 204:
   * description: Paciente removido com sucesso (No Content)
   * 404:
   * description: Paciente não encontrado (Prisma P2025)
   */
  remove: async (req: Request, res: Response) => {
    try {
      ...
      // Status 204 No Content para deleção bem-sucedida (igual ao delete RESTful)
      res.status(204).send();
    } catch (error: any) {
      if (error.code === 'P2025') {
        res.status(404).json({ error: 'Paciente não encontrado para exclusão.' });
      } else {
